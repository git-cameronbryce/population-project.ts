import { EvolvedProps, InputProps } from './command-interface/interface';
import { SlashCommandBuilder, EmbedBuilder } from 'discord.js';
import { SlashCommandProps, CommandOptions } from 'commandkit';
import axios, { AxiosResponse } from 'axios';

export const data = new SlashCommandBuilder()
  .setName('ase-search-cluster')
...
export async function run({ interaction }: SlashCommandProps) {
  ...
  const url: string = 'http://arkdedicated.com/xbox/cache/unofficialserverlist.json';
  const response: AxiosResponse<EvolvedProps[]> = await axios.get(url);

  let globalGameserverCount: number = 0;
  let currentPlayerCount: number = 0, maximumPlayerCount: number = 0, globalPlayerCount: number = 0;
  const tasks = response.data.map((server: EvolvedProps) => {
